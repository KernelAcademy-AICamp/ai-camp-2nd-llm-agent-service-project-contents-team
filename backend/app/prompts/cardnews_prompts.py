...
## ⚠️ 정확성 원칙 (매우 중요!)
**확인되지 않은 정보는 절대 작성하지 마세요!**
- 사용자가 제공하지 않은 구체적인 수치(%, 금액, 기간 등)를 임의로 만들지 않기
- 검증되지 않은 통계나 연구 결과("연구에 따르면", "전문가에 따르면" 등) 언급 금지
- 확실하지 않은 사실은 "~할 수 있습니다", "~라고 알려져 있습니다" 등 완화된 표현 사용
- 허위 정보나 과장된 효과를 약속하지 않기
...
## ⚠️ 정확성 원칙 (매우 중요!)
**확인되지 않은 정보는 절대 작성하지 마세요!**
- 사용자가 제공하지 않은 구체적인 수치(%, 금액, 날짜 등)를 임의로 만들지 않기
- 검증되지 않은 통계나 연구 결과를 언급하지 않기
- 확실하지 않은 사실은 "~할 수 있습니다", "~라고 알려져 있습니다" 등 완화된 표현 사용
- 사용자가 제공한 정보만 사실로 작성하고, 추가 정보는 일반적인 내용으로 보완
...
사용자가 "~ 하는 방법"을 질문했습니다. 실제로 도움이 되는 단계별 가이드를 작성합니다.

## ⚠️ 가장 중요한 규칙
**일반적으로 알려진 정보를 바탕으로 구체적인 방법을 제시해야 합니다!**
- 단계별로 명확하게 설명
- 실행 가능한 구체적인 액션
- 필요한 준비물/조건 명시
- 주의사항과 팁 포함
...
## ⚠️ 정확성 원칙 (매우 중요!)
**확인되지 않은 정보는 절대 작성하지 마세요!**
- 구체적인 수치(%, 금액, 시간 등)는 확실한 경우에만 사용
- "연구에 따르면", "전문가에 따르면" 등 검증되지 않은 출처 언급 금지
- 확실하지 않은 효과는 "~할 수 있습니다", "~에 도움이 될 수 있습니다" 등 완화된 표현 사용
- 과장된 효과나 허위 약속 금지
...
### 3. 페이지 수 결정 (⚠️ 한 페이지 = 한 Step 원칙!)
- **페이지 수 = 2 + Step 수** (첫 페이지 Hook + Step 페이지들 + 마지막 Tips)
- 예: Step 3개 → 최소 5장 (Hook + Step1 + Step2 + Step3 + Tips)
- 예: Step 5개 → 최소 7장 (Hook + Step1~5 + Tips)
- 원칙: **절대로 2개 이상의 Step을 한 페이지에 넣지 않기!**
...
## ⚠️ 정확성 원칙 (매우 중요!)
**확인되지 않은 정보는 절대 작성하지 마세요!**
- 제공된 "확장된 콘텐츠"와 "핵심 포인트"에 있는 정보만 사용
- 임의로 수치(%, 금액, 기간 등)를 만들어내지 않기
- 검증되지 않은 통계, 연구 결과, 전문가 의견 언급 금지
- 확실하지 않은 내용은 "~할 수 있습니다", "~에 도움이 됩니다" 등 완화된 표현 사용
- 사실이 아닌 과장된 효과나 결과를 약속하지 않기
...
## ⚠️ 정확성 원칙 (매우 중요!)
**확인되지 않은 정보는 절대 작성하지 마세요!**
- 제공된 "정보 내용"과 "핵심 포인트"에 있는 정보만 사용
- 임의로 수치(%, 금액, 기간, 통계 등)를 만들어내지 않기
- "연구 결과", "전문가에 따르면" 등 검증되지 않은 출처 언급 금지
- 확실하지 않은 내용은 "~할 수 있습니다", "~에 도움이 됩니다" 등 완화된 표현 사용
- 과장된 효과나 허위 약속 금지 (예: "100% 효과", "무조건 성공")
...
- **⚠️ 한 카드(페이지)에 반드시 하나의 Step만! (Step 1과 Step 2가 같은 카드에 있으면 안 됨)**
- **플레이스홀더 텍스트 절대 금지!**

## ⚠️ 정확성 원칙 (매우 중요!)
**확인되지 않은 정보는 절대 작성하지 마세요!**
- 제공된 "주제"와 "단계별 포인트"에 있는 정보만 사용
- 임의로 수치(%, 금액, 시간 등)를 만들어내지 않기
- "연구 결과", "전문가에 따르면" 등 검증되지 않은 출처 언급 금지
- 확실하지 않은 효과는 "~할 수 있습니다", "~에 도움이 됩니다" 등 완화된 표현 사용
- 과장된 효과나 허위 약속 금지
...
### 중간 페이지 - Steps (⚠️ 한 페이지 = 한 Step!)
- **각 페이지에 하나의 Step만 작성** (절대로 2개 이상 Step을 한 페이지에 넣지 않기!)
- 제목: "Step 1: 동작", "Step 2: 동작" 형식 (Step 번호 + 동작 요약)
- 내용: 해당 Step의 구체적인 실행 방법 (bullet points 3-5개)
- 각 bullet은 하나의 액션만
- Step이 5개면 Step 페이지도 5개 (첫 페이지, 마지막 페이지 제외)
